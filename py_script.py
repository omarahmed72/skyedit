...
with open('unit-details.html', 'r', encoding='utf-8') as f:
    content = f.read()

# Replace the content-wrapper entirely
pattern = re.compile(r'(<div class=\"content-wrapper\">)(.*?)(</div>\s*</div>\s*<!-- Footer -->)', re.DOTALL)
...
script_pattern = re.compile(r'(\/\/ --- Carousel Slider Logic ---.+?)(<\/script>\s*<\/body>)', re.DOTALL)
script_replacement = r'''// --- Dynamic Unit Logic ---
      document.addEventListener("DOMContentLoaded", () => {
        ...
        let projectLinkHTML = `<span class="detail-value">${unit.project || "N/A"}</span>`;
        if (unit.project) {
          let projects = JSON.parse(localStorage.getItem("sky_projects")) || [];
          let foundProject = projects.find(p => p.name === unit.project);
          if (foundProject) {
            projectLinkHTML = `<a href="project-details.html?id=${foundProject.id}" class="detail-value" style="color: var(--accent-gold); cursor: pointer; text-decoration: underline;">${unit.project}<i class="fa-solid fa-arrow-right ml-1" style="font-size: 10px"></i></a>`;
          } else {
             projectLinkHTML = `<span class="detail-value" style="color: var(--accent-gold);">${unit.project}</span>`;
          }
        }
        ...
        <div class="unit-content-layout">
          ...
          <div class="details-section">
            ...
            <div class="details-list">
              ...
              <div class="detail-row">
                <span class="detail-label">Compound</span>${projectLinkHTML}
              </div>
              <div class="detail-row">
...
new_content = script_pattern.sub(script_replacement, new_content)

with open('unit-details.html', 'w', encoding='utf-8') as f:
    f.write(new_content)
